...
execute unless predicate filched:item/focus/filcher/is_using_move_anchor positioned ~-2.5 ~-1 ~-2.5 as @n[type=!#filched:filcher/cannot_transport_to_limbo, tag=!filcher_focus_self, dx=4, dy=2, dz=4, predicate=!aspectlib:has_passenger_or_vehicle] at @s unless predicate aspectlib:status_effect/invisibility/any run function filched:item/focus/filcher/using/effects/passenger
# Anchor Passenger
