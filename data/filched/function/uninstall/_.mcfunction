tag @s add filched.pack_uninstaller

execute if entity @p[tag=aspectlib.receive_logs, tag=aspectlib.receive_logs_expanded, tag=!filched.pack_uninstaller] run tellraw @a[tag=aspectlib.receive_logs, tag=aspectlib.receive_logs_expanded, tag=!filched.pack_uninstaller] {text:"",color:"red",extra:[{selector:"@s",color:"gold"}," has uninstalled \"",{"text":"",color:"#6e00c9",shadow_color:-14876619,extra:["[",{translate:"filched"},"]"]},"\" from this server/world!"]}
...
tellraw @a[tag=!filched.pack_uninstaller] {text:"",color:"yellow",extra:["Please wait until an admin/owner restarts the server..."]}
tellraw @a[tag=aspectlib.receive_logs, tag=aspectlib.receive_logs_expanded, tag=filched.pack_uninstaller] ""
tellraw @a[tag=aspectlib.receive_logs, tag=aspectlib.receive_logs_expanded, tag=filched.pack_uninstaller] {text: "This action can be partially undone by restarting the server/world without removing the modification", color: "green"}
